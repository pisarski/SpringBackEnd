...
public class SpittleServiceSecurityIntegrationTest extends AbstractSecurityIntegrationTest {
	...
	public void shouldThrowExceptionForSaveNewSpittle() {
		Object[] rights = Stream.of(UserRight.values()).filter(right -> right != UserRight.ADD_SPITTLE).toArray();
	...
	public void shouldEditSpittle() {
		runWithParams(right -> {
			...
			// then
			assertTrue("No exception", true);
		}, new UserRight[] { UserRight.EDIT_ALL_SPITTLES, UserRight.EDIT_OWN_SPITTLE });
	}

	@Test
	public void shouldThrowExceptionForEditSpittle() {
		Object[] rights = Stream.of(UserRight.values())
				.filter(right -> right != UserRight.EDIT_ALL_SPITTLES && right != UserRight.EDIT_OWN_SPITTLE).toArray();
		runWithParamsAndExpectedException(right -> {
			// given
			loginWithRights((UserRight) right);
	...
	public void shouldGetSpittleById() {
		User createdUser = new UserBuilder().withId(1L)
				.withRights(Stream.of(UserRight.EDIT_OWN_SPITTLE).collect(Collectors.toSet())).build();

		User userWithEditAllRight = new UserBuilder()
				.withRights(Stream.of(UserRight.EDIT_ALL_SPITTLES).collect(Collectors.toSet())).build();
	...
	public void getByIdShouldThrowExceptionForWrongRights() {
		...
		runWithParamsAndExpectedException(right -> {
			// given
			User createdUser = new UserBuilder().withId(1L)
					.withRights(Stream.of((UserRight) right).collect(Collectors.toSet())).build();
